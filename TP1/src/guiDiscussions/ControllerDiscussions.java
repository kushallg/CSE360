...
public class ControllerDiscussions {
    ...
    protected static void addReply() {
        ...
        boolean isStaff = applicationMain.FoundationsMain.activeHomePage == 3;
        ...
        if (isStaff) {
            ...
            if (!typeResult.isPresent() || typeResult.get() == ButtonType.CANCEL) {
                return;
            }
        ...
        if (result.isPresent() && !result.get().trim().isEmpty()) {
            ...
            if ("private".equals(visibility)) {
                if (success) {
                    ...
                    ok.setTitle("Private Feedback");
                    ok.setHeaderText(null);
                    ok.setContentText("Private feedback posted!");
                    ok.showAndWait();
                } else {
                    showError("Error posting private feedback.");
                }
            } else if (!success) {
