	...
	private ViewUserUpdate() {
        ...
        button_UpdatePassword.setOnAction((event) -> {
            ...
            result.ifPresent(newPassword -> {
                ...
                String passwordErr = Model.evaluatePassword(newPassword);
                if (!passwordErr.isEmpty()) {
                    ...
                    ViewUserUpdate.alertUsernamePasswordError.setTitle("*** ERROR ***");
                    ViewUserUpdate.alertUsernamePasswordError.setHeaderText("Password Validation Error");
                    ViewUserUpdate.alertUsernamePasswordError.getDialogPane().setContent(label);
                    ViewUserUpdate.alertUsernamePasswordError.showAndWait();
                    return; // Do NOT update password if invalid
                }
        ...
        button_UpdateFirstName.setOnAction((event) -> {
            ...
            result.ifPresent(firstName -> {
                String firstNameErr = NameValidation.checkForValidName(firstName);
                if (!firstNameErr.isEmpty()) {
                    ...
                    ViewUserUpdate.alertUsernamePasswordError.setTitle("*** ERROR ***");
                    ViewUserUpdate.alertUsernamePasswordError.setHeaderText("First Name Validation Error");
                    ViewUserUpdate.alertUsernamePasswordError.getDialogPane().setContent(label);
                    ViewUserUpdate.alertUsernamePasswordError.showAndWait();
                    return;
                }
                theDatabase.updateFirstName(theUser.getUserName(), firstName);
                theDatabase.getUserAccountDetails(theUser.getUserName());
        ...
        button_UpdateMiddleName.setOnAction((event) -> {
            ...
            result.ifPresent(middleName -> {
                // Only validate if not empty
                if (!middleName.isEmpty()) {
                    String middleNameErr = NameValidation.checkForValidName(middleName);
                    if (!middleNameErr.isEmpty()) {
                        ...
                        ViewUserUpdate.alertUsernamePasswordError.setTitle("*** ERROR ***");
                        ViewUserUpdate.alertUsernamePasswordError.setHeaderText("Middle Name Validation Error");
                        ViewUserUpdate.alertUsernamePasswordError.getDialogPane().setContent(label);
                        ViewUserUpdate.alertUsernamePasswordError.showAndWait();
                        return;
                    }
                }
                theDatabase.updateMiddleName(theUser.getUserName(), middleName);
                theDatabase.getUserAccountDetails(theUser.getUserName());
        ...
        button_UpdateLastName.setOnAction((event) -> {
            ...
            result.ifPresent(lastName -> {
                String lastNameErr = NameValidation.checkForValidName(lastName);
                if (!lastNameErr.isEmpty()) {
                    ...
                    ViewUserUpdate.alertUsernamePasswordError.setTitle("*** ERROR ***");
                    ViewUserUpdate.alertUsernamePasswordError.setHeaderText("Last Name Validation Error");
                    ViewUserUpdate.alertUsernamePasswordError.getDialogPane().setContent(label);
                    ViewUserUpdate.alertUsernamePasswordError.showAndWait();
                    return;
                }
                theDatabase.updateLastName(theUser.getUserName(), lastName);
                theDatabase.getUserAccountDetails(theUser.getUserName());
        ...
        button_UpdatePreferredFirstName.setOnAction((event) -> {
            ...
            result.ifPresent(preferredFirstName -> {
                String preferredFirstNameErr = NameValidation.checkForValidName(preferredFirstName);
                if (!preferredFirstNameErr.isEmpty()) {
                    System.out.println("\n*** ERROR ***: " + preferredFirstNameErr);

                    Label label = new Label(preferredFirstNameErr);
                    label.setWrapText(true);
                    label.setMaxWidth(400);

                    ViewUserUpdate.alertUsernamePasswordError.setTitle("*** ERROR ***");
                    ViewUserUpdate.alertUsernamePasswordError.setHeaderText("Preferred First Name Validation Error");
                    ViewUserUpdate.alertUsernamePasswordError.getDialogPane().setContent(label);
                    ViewUserUpdate.alertUsernamePasswordError.showAndWait();
                    return;
                }
                theDatabase.updatePreferredFirstName(theUser.getUserName(), preferredFirstName);
                theDatabase.getUserAccountDetails(theUser.getUserName());
        ...
        button_UpdateEmailAddress.setOnAction((event) -> {
            ...
            result.ifPresent(email -> {
                String emailErr = EmailAddressRecognizer.checkEmailAddress(email);
                if (!emailErr.isEmpty()) {
                    System.out.println("\n*** ERROR ***: " + emailErr);
                    ...
                    ViewUserUpdate.alertUsernamePasswordError.setTitle("*** ERROR ***");
                    ViewUserUpdate.alertUsernamePasswordError.setHeaderText("Email Address Validation Error");
                    ViewUserUpdate.alertUsernamePasswordError.getDialogPane().setContent(label); //Display specific email error
                    ViewUserUpdate.alertUsernamePasswordError.showAndWait();
                    return;
                }
