...
/**
 * 
 * <p> AddRemoveRolesTest </p>
 *
 * <p> Description: JUnit 4 tests for ControllerAddRemoveRoles. Includes BVT and Coverage Tests.
...
public class AddRemoveRolesTest {
    ...
    private boolean addRole(EnumSet<Database.Role> set, Database.Role role) {
        if (role == null) return false;      // dropdown empty
        if (!canModify()) return false;      // not authorized
     ...
     * 	PASS: Admin was allowed to add a valid role and the role now appears in the list.
     * 	FAIL: Admin was blocked from adding the role or the role was not actually added.
     * */
    @Test
     ...
     * 	PASS: Non-admin cannot add or remove roles and the roles list remains unchanged.
     * 	FAIL: A non-admin is able to add or remove a role, which is a security violation.
     * 
     * */
    @Test
